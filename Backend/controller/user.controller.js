...
export let login = async (req , res) => {
    try{
        ...
        return res.status(200).cookie("jhaTutorialsToken" , token , {maxAge : 7 * 24 * 60 * 60 * 1000 , httpOnly:true , sameSite:process.env.NODE_ENV=="production" ? "none" : "lax" , secure:process.env.NODE_ENV=="production"}).json({
            user: user,
            success : true
        })
    }
