...
export default function AdmitCard() {
    ...
    useEffect(() => {
        const getCurrPost = async () => {
            try {
                let res = await axios.get(`${ADMITCARD_API_ENDPOINT}/get/${id}`, {
                    withCredentials: true,
                });

                if (res.data.success) {
                    setCurrPost(res.data.admitCard);
                }
            } catch (e) {
                setError(true);
            }
                                    ...
                                    <div className="max-sm:overflow-x-auto border border-gray-200 shadow-md rounded-lg my-10 max-sm:my-6">
                                        <table className="w-full text-sm border-collapse">
                                            ...
                                            <tbody className="w-[50%]">
                                                ...
                                                {/* Associated Post Link (if available) */}
                                                {currPost?.associatedPost && (
                                                    <tr>
                                                        <td className="px-4 py-2 font-semibold text-red-700 max-sm:text-xs">View Associated Post</td>
                                                        <td className="px-4 py-2 text-right max-sm:text-xs">
                                                            <Link
                                                                to={`/post/${currPost?.associatedPost?._id}`}
                                                                target="_blank"
                                                                className="underline text-blue-600 hover:text-blue-800"
                                                            >
                                                                Click Here
