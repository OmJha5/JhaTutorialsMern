...
export default function EditAnswerKey() {
    ...
    useEffect(() => {
        let getCurrAnswerKey = async () => {
            try {
                setPageLoading(true);
                let res = await axios.get(`${ANSWERKEY_API_ENDPOINT}/get/${id}`, { withCredentials: true });

                if (res.data.success) {
                    let post = res.data.answerKey;
    ...
    let submitHandler = async () => {
        if (isFormValid()) {
            try {
                ...
                let res = await axios.post(`${ANSWERKEY_API_ENDPOINT}/edit/${id}`, data, {
                    headers: {
            ...
            {error ? (
                ...
                <div className="flex h-fit flex-col md:flex-row relative">
                    ...
                    <div className="flex-1 p-6 max-sm:p-3 md:ml-80 max-md:mt-16 transition-all duration-300 ease-in-out flex gap-5 flex-col">
                        <h1 className='text-4xl max-sm:text-2xl font-medium mt-4'>Edit Answer Key</h1>
                        <div>
                            <Button onClick={() => navigate("/admin/answerkey")}><ArrowLeft size={24} /> <span className='ml-2'>Back</span></Button>
                        </div>
