...
export default function CreateAnswerKey(){
    ...
    useEffect(() => {
          document.title = "Jha Tutorials | Create New Answer Key"
    } , [])
    ...
    let submitHandler = async () => {
        if (isFormValid()) {
            try {
                ...
                let res = await axios.post(`${ANSWERKEY_API_ENDPOINT}/create`, data, {
                    headers: {
                ...
                (error) ? (
                    ...
                    <div className="flex h-fit flex-col md:flex-row relative">
                        ...
                        <div className="flex-1 p-6 max-sm:p-3 md:ml-80 max-md:mt-16 transition-all duration-300 ease-in-out flex gap-5 flex-col">
                            <h1 className='text-4xl max-sm:text-2xl font-medium mt-4'>Create New Answer Key</h1>
                            <div>
